...
export default function UserProfilePage() {
  ...
  const isFollowing = Boolean(followersQuery.data?.some((f) => f.follower.id === currentUserId));
  ...
  }}>Follow</Button>}</div>{activeList && <div className="rounded-2xl border p-4"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-semibold">{activeList === "followers" ? "Followers" : "Following"}</h2><button className="text-sm text-slate-500 hover:underline" onClick={() => setActiveList(null)}>Close</button></div><div className="space-y-2">{(activeList === "followers" ? followersQuery.data : followingQuery.data)?.length ? (activeList === "followers" ? followersQuery.data : followingQuery.data)?.map((item) => { const person = activeList === "followers" ? item.follower : item.following; const isMe = person.id === currentUserId; const iFollowPerson = Boolean(myFollowingQuery.data?.some((followItem) => followItem.following.id === person.id)); return <div key={item.id} className="flex items-center justify-between rounded-xl border p-3"><Link to={`/profile/${person.id}`} className="font-medium hover:underline">@{person.username}</Link>{isMe ? <button disabled className="rounded-lg border px-3 py-1 text-xs opacity-60">You</button> : iFollowPerson ? <button className="rounded-lg border bg-slate-700 px-3 py-1 text-xs text-white" onClick={async () => {
    try {
